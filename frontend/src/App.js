...
// ── DEEP DIVE MODAL ───────────────────────────────────────────────────────────
// Full-screen analytics view for a selected state
  ...
  return (
    <div style={{ position: 'fixed', inset: 0, zIndex: 99999, display: 'flex', flexDirection: 'column', background: 'rgba(5,7,12,0.97)', animation: 'fadeIn 0.25s ease' }}>
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '0.75rem 1.5rem', borderBottom: '1px solid var(--border)', flexShrink: 0 }}>
        ...
        <button onClick={onClose} style={{ width: 32, height: 32, borderRadius: 8, border: '1px solid var(--border)', background: 'var(--card)', color: 'var(--muted2)', fontSize: '1rem', cursor: 'pointer' }}>✕</button>
      </div>
      <div style={{ flex: 1, display: 'flex', alignItems: 'center', justifyContent: 'center', color: 'var(--muted)' }}>
        Tableau embed goes here — paste your public.tableau.com embed code
      </div>
    </div>
  );
}
...
export default function App() {
  ...
  const [compareState,   setCompareState]   = useState(null); // second state for side-by-side compare
  ...
  const selectState = useCallback(async stateName => {
    // if nothing selected yet — set as primary
    if (!selected) {
      const data = await fetchState(stateName);
      if (data) setSelected(data);
      return;
    }
    // clicking the already-selected state → deselect
    if (selected.state === stateName) {
      setSelected(compareState);
      setCompareState(null);
      return;
    }
    // clicking the compare state → deselect compare
    if (compareState?.state === stateName) {
      setCompareState(null);
      return;
    }
    // a different state is clicked while one is selected → compare mode
    const data = await fetchState(stateName);
  ...
  // search handler — when called with (stateName, true) it means user selected from dropdown
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', height: '100vh', overflow: 'hidden' }}>
      ...
      <div style={{ display: 'flex', flex: 1, overflow: 'hidden' }}>
        ...
        {/* Sidebar — shows compare panel when two states selected, normal sidebar otherwise */}
        <div style={{ width: 300, flexShrink: 0, background: 'var(--surface)', borderRight: '1px solid var(--border)', display: 'flex', flexDirection: 'column', overflow: 'hidden', position: 'relative' }}>
          {compareState ? (
            ...
            <Sidebar
              ...
              onClose={handleReset}
              onStateDetail={() => {}}
              onDeepDive={() => setDeepDiveModal(true)}
            />
          )}
        </div>
        ...
        <div style={{ flex: 1, position: 'relative' }}>
          ...
          {/* Hint — shown when one state selected, before compare mode kicks in */}
